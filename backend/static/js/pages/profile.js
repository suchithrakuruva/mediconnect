...
function switchProfileTab(tab) {
    ...
    if (tab === 'appt') {
        api.profile.appointments().then(appts => {
            ...
            c.innerHTML = `<div class="table-wrap"><table><thead><tr><th>Doctor</th><th>Date</th><th>Time</th><th>Type</th><th>Status</th><th>Amount</th></tr></thead><tbody>${appts.map(a => `<tr><td class="font-bold">Doctor #${a.doctor_id}</td><td>${formatDate(a.appointment_date)}</td><td>${a.appointment_time}</td><td><span class="badge badge-primary">${a.type}</span></td><td><span class="badge badge-${a.status === 'completed' ? 'success' : a.status === 'cancelled' ? 'danger' : 'info'}">${a.status}</span></td><td>₹${a.amount || 0}</td></tr>`).join('')
                }</tbody></table></div>`;
        });
    } else if (tab === 'txn') {
...
function editProfile() {
    api.profile.get().then(u => {
        createModal('✏️ Edit Profile', `
          ...
          <button class="btn btn-primary btn-full" onclick="saveProfile()"><i class="fas fa-save"></i> Save Changes</button>
        `);
    });
}
